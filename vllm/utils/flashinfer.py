...
# Setter/getter for FlashInfer's autotune-rank-sync hook. Fallback is a no-op
# pair so callers can always call them regardless of FlashInfer version; the
# actual opt-in is gated by ``has_flashinfer_autotune_process_group()`` and
# the ``VLLM_FLASHINFER_AUTOTUNE_PROCESS_GROUP`` env var.
