...
def has_flashinfer_autotune_process_group() -> bool:
    ...
    When available, callers can wire it around the warmup autotune block so
    that per-rank tactic choice is synchronised across the TP/EP group --
    fixes the same family of rank-desync bugs as the KV-offload barrier.
    """
    if not has_flashinfer():
        return False
