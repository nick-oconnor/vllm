...
def _run_flashinfer_sparse_mla_decode_autotune(
    ...
    # Optional cross-rank tactic sync: make the autotuner all-reduce measured
    # timings across the TP group before choosing a config, so every rank
    # picks the same tactic. Opt in via VLLM_FLASHINFER_AUTOTUNE_PROCESS_GROUP
    # and force every rank into the autotune context (the leader-only path
    # would deadlock the leader's all-reduce waiting for non-leaders that
    # never enter).
    ...
    try:
        with torch.inference_mode():
            ...
            # context (the process-group all-reduce deadlocks if only the
            # leader participates).
